...
// `attributes: [...]` is moved to the end to match the order PHPX uses by default
$favIcon = $x->render($x->svg(xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 24 24', fill: 'white', c: $x->path(d: 'M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-1.72 6.97a.75.75 0 1 0-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 1 0 1.06 1.06L12 13.06l1.72 1.72a.75.75 0 1 0 1.06-1.06L13.06 12l1.72-1.72a.75.75 0 1 0-1.06-1.06L12 10.94l-1.72-1.72Z', attributes: ['fill-rule' => 'evenodd', 'clip-rule' => 'evenodd'])));
...
// $x->out is not supported by phpx-compile
return $x->render($x->html(c: [
    $x->head(c: [
        ...
        $x->title(c: 'PHPX: PHP DOM but intuitive'),
    ]),
    $x->body(c: [
        $x->div(class: 'flex justify-center h-screen', c: [
            // Introduction half
            $x->div(class: 'md:w-1/2 prose-sm text-xs sm:text-sm md:text-base flex flex-column justify-center items-center p-6', c: $x->div(c: [
                ...
                $x->sup(class: 'mb-2', c: $x->em(c: 'PHP DOM but intuitive')),
                $x->code(class: 'block', c: $x->raw(highlight_string("<?= \$x->render(\$x->sup(c: \$x->em(c: 'Hey')))", true))),
            ...
            // Source code half
            $x->div(class: 'w-1/2 text-sm bg-gray-200 min-h-screen w-50 hidden lg:flex items-center font-mono', c: [
                $x->div(class: 'flex mr-2 overflow-scroll h-screen', c: [
                    // Convenient variable capture method
                    ...$x->with('<?php echo "hello world";', fn(string $contents) => [
